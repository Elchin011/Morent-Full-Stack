                ...
                <div className="flex flex-col py-8 pl-6 pr-2 w-64 bg-white flex-shrink-0">
                    ...
                    <div className="flex flex-col items-center bg-indigo-100 border border-gray-200 mt-4 w-full py-6 px-4 rounded-lg">
                        <div className="h-20 w-20 rounded-full border overflow-hidden">
                            <img src="https://i.pinimg.com/736x/09/21/fc/0921fc87aa989330b8d403014bf4f340.jpg" alt="Avatar" className="h-full w-full" />
                        </div>
                        ...
                        <div className="flex flex-col space-y-1 mt-4 -mx-2 h-48 overflow-y-auto">
                            {conversations.map((conversation) => (
                                <Link to={paths.DASHBOARD.CHAT.USER(conversation._id)}
                                    ...
                                    style={{ backgroundColor: id === conversation._id ? "#f1f1f1" : "" }}
                                >
...
const MessageItem = ({ message, owner }: { message: string, owner: boolean }) => {
    return (
        <div className={cn("p-3 rounded-lg", owner ? "col-start-6 col-end-13" : "col-start-1 col-end-8")}>
            <div className={cn("flex items-center", owner && "justify-start flex-row-reverse")}>
                <div className="flex items-center justify-center h-10 w-10 rounded-full bg-indigo-500 flex-shrink-0">
                    A
                </div>
                <div className={cn("relative ml-3 text-sm py-2 px-4 shadow rounded-xl", owner ? "bg-indigo-100 mr-3" : "bg-white mr-2")}>
                    <div>{message}</div>
                </div>
            </div>
        </div>
