...
export const PaymentSummary = ({ rent }: Props) => {
    const { name, images, price, discount, reviews } = rent
    ...
    const avgRating = reviewCount > 0
        ...
        : 0
    return (
        <div className="rounded-[10px] bg-white p-4 lg:p-6 h-fit lg:sticky top-[160px] mx-4 md:mx-0">
            ...
            <div className="flex items-center gap-x-4">
                ...
                <div>
                    ...
                    <div className="mt-2 flex items-center gap-x-2">
                        <ReviewStar rating={avgRating} />
                        <p className="text-secondary text-sm font-medium tracking-[-0.28px]">{reviewCount}  Reviewer</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h4 className="text-secondary-500 text-[16px] lg:text-xl font-bold !leading-[-150%] tracking-[-0.6px]">Total Rental Price</h4>
                    <p className="text-sm font-medium leading-[150%] tracking-[-0.28px] text-secondary-300">Overall price rental</p>
                </div>
                <p className="text-secondary-500 text-[20px] lg:text-[32px] !leading-normal font-bold">{formatPrice(price - discount)}</p>
            </div>
        </div>
    )
}
