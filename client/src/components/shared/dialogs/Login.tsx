...
export const LoginDialog = () => {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: (error: AxiosError<AuthResponseType>) => {
            ...
            const msg = typeof data === "object" && data !== null && "message" in data ? (data as { message: string }).message : null;
            const message = msg ?? (error.response ? `Xəta ${error.response.status}` : "Serverə çatılmadı. Backend işləyir? (port 3000)") ?? "Xəta baş verdi. Yenidən cəhd edin.";
            toast.error(message);
        }
    })
